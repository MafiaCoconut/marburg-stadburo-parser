...
class AdressanderungParserInterface(BaseParserInterfaceImpl):
    def __init__(self):
        super().__init__()
        self.waiting_time = 2
        self.category_id = 1
    ...
    def get_termins(self) -> dict:
        items = self.driver.find_element(By.XPATH, '//*[@id="button-plus-20"]')
        items.click()
        ...
        for d in range(1, count_days+1):
            ...
            result.append(self.get_termins_per_day(day_id=day_id, day=day))
            if flag:
                break
    ...
    def get_termins_per_day(self, day_id: int, day: str):
        termins = []
