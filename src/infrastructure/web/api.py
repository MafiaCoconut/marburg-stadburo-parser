...
@router.get("/category_of_termins/getData")
async def get_text_category_of_termins_data(category_of_termins: int):
    """
    Функция возвращает по api данные о категории и текущих терминах
    :param category_of_termins: ID категории в базе данных
    :return: {'termins': list[Termin], 'category_of_termins': CategoryOfTermins, 'error': None}
    """
    return await termins_service.get_category_of_termins_data(category_of_termins=category_of_termins)


@router.put('category_of_termins/deactivate')
...
@router.put('category_of_termins/reactivate')
